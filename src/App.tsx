...
export default function App() {
      ...
      <div className="h-screen w-screen bg-[#020617] flex items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col items-center gap-2">
            <h1 className="text-white text-xl font-black uppercase tracking-[0.3em] animate-pulse">
              Aetherium AI
            </h1>
            <p className="text-slate-600 font-bold uppercase tracking-widest text-[10px]">
              Neural Fiscal OS v2.0
            </p>
          </div>
        </div>
      </div>
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case "audit":
        return (
          <FiscalAuditView companyId={selectedCompanyId!} apiFetch={apiFetch} />
        );
      case "lex-db":
        return (
          <LegalLibraryView companyId={selectedCompanyId} apiFetch={apiFetch} />
        );
      case "e-invoicing":
        return (
          <EInvoicingView companyId={selectedCompanyId!} apiFetch={apiFetch} />
        );
      case "rnc-search":
      ...
      case "knowledge-base":
        return (
          <KnowledgeBase apiFetch={apiFetch} companyId={selectedCompanyId} />
        );
      case "payroll":
        return (
          <PayrollView companyId={selectedCompanyId!} apiFetch={apiFetch} />
        );
      case "assets":
        return (
          <FixedAssetsView companyId={selectedCompanyId!} apiFetch={apiFetch} />
        );
      case "registries":
        return (
          <TaxRegistryView companyId={selectedCompanyId!} apiFetch={apiFetch} />
        );
      case "ncf":
        return (
          <NCFManagementView
    ...
    <TrialGuard>
      ...
      <div className="min-h-screen bg-[#020617] text-slate-200 font-inter selection:bg-indigo-500/30">
        ...
        <motion.aside
          ...
          className="fixed left-0 top-0 h-full bg-[#030816]/95 backdrop-blur-3xl border-r border-white/5 z-50 flex flex-col shadow-2xl transition-all duration-300 overflow-hidden"
        >
          <div className="p-6 flex items-center justify-between border-b border-white/5">
            <div
              className={`flex items-center gap-3 ${isSidebarOpen || window.innerWidth < 1024 ? "flex" : "hidden"}`}
            >
            ...
            <button
              ...
              className="p-2.5 hover:bg-white/5 rounded-xl text-slate-500 hover:text-white transition-all border border-white/5"
            >
          ...
          <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto custom-scrollbar">
            <div
              className={`px-4 pb-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ${!isSidebarOpen && "hidden"}`}
            >
              Operativa
            </div>
            <NavItem
            ...
            <div
              className={`pt-8 pb-4 px-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ${!isSidebarOpen && "hidden"}`}
            >
              Cumplimiento
            </div>
          ...
          <div className="p-4 border-t border-white/5">
            <div
              className={`p-4 bg-gradient-to-br from-indigo-600/10 to-violet-600/10 border border-indigo-500/20 rounded-2xl mb-4 ${!isSidebarOpen && "hidden"}`}
            >
              ...
              <p className="text-[9px] text-slate-500 leading-relaxed font-medium">
                Dominican Fiscal Intelligence active for {user?.fullName}.
              </p>
            </div>
            <button
              onClick={logout}
        ...
        {/* Main Content */}
        <main
          className={`transition-all duration-300 min-h-screen ${isSidebarOpen ? "lg:pl-[280px]" : "lg:pl-[80px]"} pl-0`}
        >
          {/* Top Header */}
          <header className="h-20 bg-[#020617]/80 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 md:px-10 sticky top-0 z-40">
            <div className="flex items-center gap-4">
              <button
                onClick={() => setIsSidebarOpen(true)}
                className="p-2 bg-white/5 rounded-xl lg:hidden text-slate-400"
              >
                <Menu size={20} />
              </button>
              {company && (
                <div className="hidden sm:flex items-center gap-2 px-4 py-1.5 bg-indigo-500/10 border border-indigo-500/20 rounded-full">
                  <Building2 size={14} className="text-indigo-400" />
                  <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400 truncate max-w-[100px] md:max-w-none">
                    {company.name}
                  </span>
                </div>
              )}
            ...
            <div className="flex items-center gap-3 md:gap-6">
              <div className="relative hidden md:block">
                ...
                <input
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-xs focus:ring-1 focus:ring-indigo-500 outline-none w-64"
                />
              </div>
              <button
              ...
              <div
                ...
                className={`flex items-center gap-3 p-1.5 md:pr-4 rounded-2xl border transition-all cursor-pointer group ${currentPage === "profile" ? "bg-indigo-600 border-indigo-400 shadow-lg shadow-indigo-600/20" : "bg-white/5 border-white/5 hover:bg-white/10"}`}
              >
                ...
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest group-hover:text-indigo-300 transition-colors">
                    {user?.role}
                  </p>
                </div>
              </div>
...
function NavItem({ active, onClick, icon, label, collapsed, locked }: any) {
  return (
    ...
    >
      ...
      {locked && !collapsed && (
        <span className="ml-auto text-[8px] bg-slate-800 px-1.5 py-0.5 rounded uppercase">
          Seleccionar
        </span>
      )}
      {active && (
      ...
      return (
        <div className="p-10 bg-rose-500/10 border border-rose-500/20 rounded-[2rem] text-center">
          ...
          <h1 className="text-2xl font-black text-white uppercase tracking-tight mb-2">
            Colapso de Componente Neural
          </h1>
          <p className="text-slate-400 mb-6 font-medium text-sm">
            El subsistema gráfico ha detectado una excepción severa y ha
            detenido el renderizado para proteger la memoria.
          </p>
